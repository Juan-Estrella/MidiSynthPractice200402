...
void Martone::LfoUpdate(bool retrig, u8 mode, float FILtop, float FILbottom, u8 m_str, u8 m_osc)
{
  ...
  static u8 waveFunction1[m_NUM_STRINGS][m_NUM_OSC];
  static u8 waveFunction2[m_NUM_STRINGS][m_NUM_OSC];
  unsigned long maxLfoCount[m_NUM_STRINGS][m_NUM_OSC];
  ...
  for (int st = 0; st < 4; st++)
  {
    for (int osc = 0; osc < 4; osc++)
    {
      ...
      waveFunction1[st][osc] = {1};
      waveFunction2[st][osc] = {2};
      maxLfoCount[st][osc] = 3000000;
    }
  }
